...
class DeviceButtonTests: XCTestCase {
  ...
  func testSizeThatFitsShortTitle() {
    XCTAssertEqual(
      ...
      CGSize(width: 247, height: 108),
      "Should provide the expected size that fits for a given title"
    )
  }
  ...
  func testSizeThatFitsAnEmptyTitle() {
    XCTAssertEqual(
      ...
      "Should provide the expected size that fits for an empty title"
    )
  }
}
