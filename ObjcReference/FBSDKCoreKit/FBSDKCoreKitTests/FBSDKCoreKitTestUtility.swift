...
class FBSDKCoreKitTestUtility: NSObject {
    /**
    ...
    class func mainBundleMock() -> Any? {
        ...
        let correctMainBundle = Bundle(for: FBSDKCoreKitTestUtility.self)
        mockNSBundle?.stub().classMethod().andReturn(correctMainBundle).main
        return mockNSBundle
    }

    class func mockAccountStoreAdapter() -> Any? {
        let mockAccountStoreAdapter = OCMockObject.partialMock(forObject: FBSDKSystemAccountStoreAdapter.sharedInstance())
