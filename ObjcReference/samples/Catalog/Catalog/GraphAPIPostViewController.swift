...
// MARK: - Post Checkin
    @IBAction func postCheckin(_ sender: Any) {
        let postCheckinCompletion: (() -> Void)? = {
...
// MARK: - Helper Method
    func handleRequestCompletion(withResult result: Any?) throws {
        ...
        if error != nil {
            ...
            if let error = error {
                message = "Post failed with error:\n \(error)"
            }
        } else {
            title = "Graph Request Success"
            if let result = result {
                message = "Post success with result:\n \(result)"
            }
        }
