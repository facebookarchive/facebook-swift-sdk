...
class CustomLoggingViewController: UIViewController {
    @IBOutlet private weak var eventField: UITextField!

    @IBAction func logCustomEvent(_ sender: Any) {
        var alertController: UIAlertController?
        if (eventField.text?.count ?? 0) <= 0 {
            alertController = AlertControllerUtility.alertController(withTitle: "Invalid event", message: "Event cannot be empty.")
            if let alertController = alertController {
                present(alertController, animated: true)
            }
        } else {
            FBSDKAppEvents.logEvent(eventField.text ?? "")
