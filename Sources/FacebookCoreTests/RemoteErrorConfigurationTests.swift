...
class RemoteErrorConfigurationTests: XCTestCase {
  private let decoder = JSONDecoder()

  // MARK: Creating Single Error Configuration
  func testCreatingWithEmptyDictionary() {
    do {
      ...
      XCTAssertNotNil(error as? DecodingError,
                      "Should throw a decoding error when trying to create a remote error configuration from an empty dictionary")
    }
  }
  ...
  func testCreatingWithValidInputs() {
    ...
    XCTAssertEqual(config.name, SampleRawRemoteConfiguration.name,
  ...
  func testCreatingFromJSON() {
    ...
    XCTAssertNotNil(try decoder.decode(RemoteErrorRecoveryConfigurationList.self, from: data),
