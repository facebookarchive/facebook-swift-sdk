...
{
  NSString *appID = (Settings_ObjC.shared.appIdentifier ?: @"");
  NSString *suffix = (Settings_ObjC.shared.urlSchemeSuffix ?: @"");
...
+ (NSBundle *)bundleForStrings
{
  ...
  dispatch_once(&onceToken, ^{
    NSString *stringsBundlePath = [[NSBundle bundleForClass:[FBApplicationDelegate_ObjC class]]
                               ...
                               error:(NSError *__autoreleasing *)errorRef
{
  ...
  NSString *domainPart = Settings_ObjC.shared.domainPrefix;
  if (domainPart.length) {
    host = [[NSString alloc] initWithFormat:@"%@.%@", domainPart, host];
  }
  host = [NSString stringWithFormat:@"%@%@", hostPrefix ?: @"", host ?: @""];

  NSString *version = (defaultVersion.length > 0) ? defaultVersion : [Settings_ObjC.shared.graphAPIVersion_ObjC description];
  if (version.length) {
  ...
  if (path.length) {
    ...
    if ([versionScanner scanString:@"/v" intoString:NULL] &&
      ...
      NSString *graphVersion = [[Settings_ObjC.shared graphAPIVersion_ObjC] description];
      NSString *reason = [NSString stringWithFormat:@"Invalid Graph API version:%@, assuming %@ instead", version, graphVersion];
      [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors message:reason];
...
+ (void)unregisterTransientObject:(__weak id)object
{
  ...
  if (count == 1) {
    ...
    NSString *message = [NSString stringWithFormat:@"unregisterTransientObject:%@ count is 0. This may indicate a bug in the FBSDK. Please"
                         " file a report to developers.facebook.com/bugs if you encounter any problems. Thanks!", [object class]];
    [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors message:message];
  }
}
...
+ (void)validateAppID
{
  if (![Settings_ObjC.shared appIdentifier]) {
...
+ (NSString *)validateRequiredClientAccessToken {
  if (![Settings_ObjC.shared clientToken]) {
  ...
  return [NSString stringWithFormat:@"%@|%@", Settings_ObjC.shared.appIdentifier, Settings_ObjC.shared.clientToken];
}

+ (void)validateURLSchemes
{
  [self validateAppID];
  NSString *defaultUrlScheme = [NSString stringWithFormat:@"fb%@%@", [Settings_ObjC.shared appIdentifier], [Settings_ObjC.shared urlSchemeSuffix] ?: @""];
...
+ (UIWindow *)findWindow
{
  ...
  if (window == nil) {
    [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors
                    message:@"Unable to find a valid UIWindow"];
  }
  return window;
}
...
+ (UIViewController *)topMostViewController
{
  ...
  if (keyWindow !=  nil && !keyWindow.isKeyWindow) {
    NSString *logString = [NSString stringWithFormat:@"Unable to obtain a key window, marking %@ as keyWindow", keyWindow.description];
    [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors
                    message:logString];
