...
#import "Internal/LoggingBehaviorConstants.h"
...
@interface FBSDKBridgeAPI() <ApplicationObserving, FBSDKContainerViewControllerDelegate>

@end

@implementation FBSDKBridgeAPI {
  BridgeAPIRequest_ObjC *_pendingRequest;

  FBSDKBridgeAPIResponseBlock _pendingRequestCompletionBlock;
  __weak id<URLOpening_ObjC> _pendingURLOpen;
...
+ (void)load
{
  FBSDKBridgeAPI *bridgeAPI = [FBSDKBridgeAPI sharedInstance];
  AnyApplicationObserving_ObjC *wrappedObserver = [[AnyApplicationObserving_ObjC alloc] initWithApplicationObserving: bridgeAPI];
  [FBApplicationDelegate_ObjC.shared addObserver: wrappedObserver];
}
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
  ...
  if (notExpectingBackground) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName: NotificationNameConstants.didBecomeActiveNotification object:self];
  }
}
...
{
  id<URLOpening_ObjC> pendingURLOpen = _pendingURLOpen;
...
didFinishLaunchingWithOptions:(NSDictionary<UIApplicationLaunchOptionsKey, id> *)launchOptions
{
  ...
  if (launchedURL &&
    ...
    if (loginManagerClass) {
      ...
      id<URLOpening_ObjC> loginManager = [[loginManagerClass alloc] init];
...
- (BOOL)isEqualTo:(id<ApplicationObserving> _Nonnull)application { 
  return [self isEqual:application];
}

#pragma mark - Internal Methods

- (void)openURL:(NSURL *)url sender:(id<URLOpening_ObjC>)sender handler:(FBSDKSuccessBlock)handler
{
...
- (void)openBridgeAPIRequest:(BridgeAPIRequest_ObjC *)request
             ...
             completionBlock:(FBSDKBridgeAPIResponseBlock)completionBlock
{
  ...
  NSError *error;
  NSURL *requestURL = [request requestURLAndReturnError:&error];
  if (!requestURL) {
...
- (void)openURLWithSafariViewController:(NSURL *)url
                                 sender:(id<URLOpening_ObjC>)sender
                     fromViewController:(UIViewController *)fromViewController
                                handler:(FBSDKSuccessBlock)handler
{
  ...
  if (SFSafariViewControllerClass) {
    ...
    if (parent == nil) {
      [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors
                      message:@"There are no valid ViewController to present SafariViewController with"];
      return;
    }
...
- (void)_openURLWithAuthenticationSession:(NSURL *)url
{
  ...
  if (AuthenticationSessionClass != nil) {
    if (_authenticationSession != nil) {
      [Logger_ObjC.shared log:FBSDKLoggingBehaviorDeveloperErrors
                      message:@"There is already a request for authenticated session. Cancelling active SFAuthenticationSession before starting the new one."];
      [_authenticationSession cancel];
    }
...
- (void)safariViewControllerDidFinish:(UIViewController *)safariViewController
{
  if (_pendingURLOpen) {
    ...
    [pendingURLOpen application:UIApplication.sharedApplication
                        openUrl:nil
...
{
  BridgeAPIRequest_ObjC *request = _pendingRequest;
