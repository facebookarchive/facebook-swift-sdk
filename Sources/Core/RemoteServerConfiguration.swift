...
extension Remote {
  ...
  typealias EventBindingList = [EventBinding]
  ...
  struct ServerConfiguration: Decodable {
    ...
    let dialogConfigurations: Remote.DialogConfigurationList?
    let dialogFlows: DialogFlowList?
    let errorConfiguration: ErrorConfigurationEntryList?
    ...
    let eventBindings: EventBindingList?
    let restrictiveRules: [RestrictiveRule]?
    let restrictiveEventParameterList: RestrictiveEventParameterList?
    ...
    init(from decoder: Decoder) throws {
      ...
      self.dialogConfigurations = try? container.decode(DialogConfigurationList.self, forKey: .dialogConfigurations)
      self.dialogFlows = try? container.decode(DialogFlowList.self, forKey: .dialogFlows)
      self.errorConfiguration = try? container.decode(ErrorConfigurationEntryList.self, forKey: .errorConfiguration)
      ...
      self.restrictiveRules = try? container.decode([RestrictiveRule].self, forKey: .restrictiveRules)
      self.restrictiveEventParameterList = try? container.decode(
        RestrictiveEventParameterList.self,
